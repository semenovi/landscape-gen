...
export const upliftStreamPowerGenerator: HeightfieldGenerator<UpliftStreamPowerParams> = {
  meta: {
    ...
    note:
      "На каждом шаге вся внутренняя область поднимается на upliftRate, затем реки (D8 сток по " +
      "накопленной площади водосбора) срезают рельеф по степенному закону stream power, K умножить на A " +
      "в степени m умножить на S в степени n. Границы сетки закреплены на нулевой высоте как базис. " +
      "В отличие от шума, рельеф не задаётся напрямую, а вырастает из повторяющегося цикла поднятия " +
      "и эрозии, поэтому реки заведомо согласованы с водоразделами.",
  },
  params,
  generate(width, depth, p): Heightfield {
    ...
    for (let z = 0; z < depth; z++) {
      for (let x = 0; x < width; x++) {
        data[z * width + x] = isBoundaryCell(x, z, width, depth) ? 0 : (rng() - 0.5) * p.initialRoughness;
      }
    }
